...
class Calendar extends React.Component {
  ...
  getNumberOfWeeks() {
    ...
    if (numDaysInMonth == 28)
      if (firstDayOfMonth > 0) numWeeks = 5;
      else numWeeks = 4;
    else if (numDaysInMonth == 29)
      if (firstDayOfMonth > 6) numWeeks = 6;
      else numWeeks = 5;
    else if (numDaysInMonth == 30)
      if (firstDayOfMonth > 6) numWeeks = 6;
      else numWeeks = 5;
    // Days == 31
    else if (firstDayOfMonth > 5) numWeeks = 6;
    else numWeeks = 5;
  ...
  getWeekData() {
    if (this.state.weeksStartEnd.length <= 0) {
...

      {
        ...
        this.state.weeksStartEnd.push({
          SundayDate: iSunday,
          SaturdayDate: (iLoop = iSaturday),
        });
      }

      for (
        var iWeekLength = this.getNumberOfWeeks(), iWeekLoop = 1;
        iWeekLoop < iWeekLength;
        iWeekLoop++
      )
        this.state.weeksStartEnd.push({
          SundayDate: iLoop + 1,
          SaturdayDate: (iLoop += 7),
        });
    }
  ...
  setCurrentDate(year, month, day) {
    ...
    if (typeof day == "undefined")
      dt = new Date(
        this.state.currentDate.getFullYear() + year,
        this.state.currentDate.getMonth() + month
      );
    else
      dt = new Date(
        this.state.currentDate.getFullYear() + year,
        this.state.currentDate.getMonth() + month,
        this.state.currentDate.getDate() + day
      );
        ...
        <span className="link scrollArrow">
          ...
          <map name="month_scrollmap">
            <area
              ...
              coords="0,0,16,7"
              href="#"
              onClick={(event) => this.monthUpClick(event)}
              shape="rect"
            />
            <area
              alt="down-arrow"
              coords="0,8,16,16"
              href="#"
              onClick={(event) => this.monthDownClick(event)}
              shape="rect"
            />
          </map>
        </span>
        ,{" "}
        <span className="link" id="thNow_year">
          {this.state.calendarRef.getCurrentDate().getFullYear()}
        </span>
        <span className="link scrollArrow">
          ...
          <map name="year_scrollmap">
            <area
              ...
              coords="0,0,16,7"
              href="#"
              onClick={(event) => this.yearUpClick(event)}
              shape="rect"
            />
            <area
              alt="down-arrow"
              coords="0,8,16,16"
              href="#"
              onClick={(event) => this.yearDownClick(event)}
              shape="rect"
            />
          </map>
        </span>
...
class MonthDate extends React.Component {
  ...
  render() {
    ...
    else return <td style={{ textAlign: "center" }}>&nbsp;</td>;
  }
}
...
class MonthWeek extends React.Component {
  ...
  render() {
    ...
    for (
      var iSaturday = this.state.saturday, iLoop = this.state.sunday;
      iLoop <= iSaturday;
      iLoop++
    )
      if (iLoop > this.getLastDateOfMonth(this.state.month) || iLoop <= 0)
        dates.push(null);
      else dates.push(iLoop);
  ...
  render() {
    return (
      <th
        ...
        onClick={(event) => this.funcNextMonthClick(event)}
      >
        &gt;&gt;
      </th>
    );
...
class PreviousMonthHeader extends React.Component {
  ...
  render() {
    return (
      <th
        style={{ cursor: "pointer" }}
        onClick={(event) => this.funcPrevMonthClick(event)}
      >
        &lt;&lt;
      </th>
    );
